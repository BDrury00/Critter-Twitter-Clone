...
When navigating to the root URL `/`, the user will see a list of tweets from the accounts that the current user follows.
...
When clicking the "like" button, it will increment the # of likes. Clicking again will "unlike" the tweet.
...
It will show up in the feed below after posting.
...
Twitter allows tweets up to 280 characters. The app should display a "remaining characters" indicator, which shifts colors as the user approaches/surpasses the limit:

![Character counter](./assets/screenshots/character-count.gif)


- Become yellow when 80% of the limit is used up (55 characters remaining)
- Become red when the number dips into the negatives.
- Not able to submit a tweet that has exceeded the limit.
...
For the GET endpoints, there is an error screen that will show it if the request fails:
